...
async function loadDishes() {
    showLoading();
    try {
        const dishes = await apiRequest('/dishes?include=chef,ratings');
        allDishes = dishes;
...
function createDishCard(dish) {
    ...
    const averageRating = calculateAverageRating(dish);
    const ratingCount = dish.ratings ? dish.ratings.length : 0;
...
function calculateAverageRating(dish) {
    ...
    const sum = dish.ratings.reduce((total, rating) => total + rating.rating, 0);
    return sum / dish.ratings.length;
}

// Get user's rating for a dish
function getUserRating(dish) {
    if (!isAuthenticated() || !dish.ratings) return null;
    
    const userId = getCurrentUser().id;
    return dish.ratings.find(rating => rating.userId === userId);
}
...
function sortDishes(dishes, sortBy) {
    switch(sortBy) {
        ...
        case 'popular':
        default:
            return dishes.sort((a, b) => (b.ratings?.length || 0) - (a.ratings?.length || 0));
    }
}
